...
export class AgentsService extends TableService<UserModel> implements OnDestroy {
    ...
    find(tableState: ITableState): Observable<TableResponseModel<UserModel>> {
        ...
        return this._userservicedomain.getInternalUsers(page , size, docnum, name, state, "USEREXT").pipe(
        map(
        response => {
