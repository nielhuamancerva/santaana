...
@Component({
  selector: 'app-hasher-agent-modal',
  templateUrl: './hasher-agent-modal.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush
})

export class HasherAgentModalComponent implements OnInit, OnDestroy{
    ...
    save() {
        ...
        const httpResult$ = this.agentService.changeHasherRoot(this.$_user.id, formData.hasher, this.$_user).subscribe( response  => {
            if(response.success) {
                this.modal.close();
            }   
        });
